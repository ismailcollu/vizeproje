...
Public Class ManageController
    ...
    Public Async Function Index(message As System.Nullable(Of ManageMessageId)) As Task(Of ActionResult)
        ViewBag.StatusMessage = If(message = ManageMessageId.ChangePasswordSuccess, "Parolanız değiştirildi.", If(message = ManageMessageId.SetPasswordSuccess, "Parolanız ayarlandı.", If(message = ManageMessageId.SetTwoFactorSuccess, "İki öğeli kimlik doğrulama sağlayıcısı ayarlandı.", If(message = ManageMessageId.[Error], "Hata oluştu.", If(message = ManageMessageId.AddPhoneSuccess, "Telefon numaranız eklendi.", If(message = ManageMessageId.RemovePhoneSuccess, "Telefon numaranız kaldırıldı.", ""))))))
    ...
    Public Async Function AddPhoneNumber(model As AddPhoneNumberViewModel) As Task(Of ActionResult)
        ...
        If UserManager.SmsService IsNot Nothing Then
            Dim message = New IdentityMessage() With {
                ...
                .Body = "Güvenlik kodunuz: " & Convert.ToString(code)
            }
    ...
    Public Async Function ManageLogins(message As System.Nullable(Of ManageMessageId)) As Task(Of ActionResult)
        ViewBag.StatusMessage = If(message = ManageMessageId.RemoveLoginSuccess, "Dış oturum kaldırıldı.", If(message = ManageMessageId.[Error], "Hata oluştu.", ""))
        Dim userInfo = Await UserManager.FindByIdAsync(User.Identity.GetUserId())
